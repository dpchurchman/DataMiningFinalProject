...
=======
Using a graph plot for these relationships reveals similar information as the parallel coordinates plot. The most central nodes are "adult", "male", and "unharmed, arrested." One interesting part of the graph is that "female" appears to only be connected to the "significant other" relationship.
...
Using the arules packages, we were able to develop 28 association rules for suspects in shooting incidents, which we graphed above. This is not too large a number to look at the rule set in their entirety:

```{r insectrhsrules}
inspect(head(suspectRules, n=28,by="confidence"))
```

The 1st, 4th, and 11th highest rules in terms of confidence involved armed robbery. Of these three rules, two associated armed robbery with the adult age group. All three resulted in the suspect being unharmed, though one also included "arrested". In conjunction, this tells us that armed robbery is predominately committed by adults, and is relatively low risk for the suspect.

The 2nd, 3rd, 5th, 6th, 8th, 10th, 12th, and 14th rules according to confidence all involved the suspect being unharmed and arrested. These mostly included family or significant other relationships and adult age groups, though notably the 2nd highest rule associates teenagers. These rules in conjunction indicate that suspects of shootings in this database largely go unharmed, though are also arrested. The large number of these rules that include victims the suspect knows also indicates that most shootings involve someone the suspect knows, which in turn could also explain why the suspect is usually arrested.  

# Modeling and Evaluation 2
<!-- [10 points] Evaluate and Compare -->
>>>>>>> f7e22d50ac4a0febcb3f12f5b89125e0ba42ed9d
